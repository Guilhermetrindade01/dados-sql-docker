...
def create_dataframe(path_name: str) -> pd.DataFrame:
    ...
    logging.info(f"\n DataFrame criado com {len(df)} linhas(s)")
    return df
...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(
        f"\n Coluna 'weather' normalizada - {len(df_weather.columns)} colunas)")
    return df


def drop_columns(df: pd.DataFrame, columns_name: list) -> pd.DataFrame:
...
def rename_columns(df: pd.DataFrame, columns_names: dict[str, str]) -> pd.DataFrame:
    logging.info(f"\n-> Renomeadas: {len(df.columns)} colunas...")
...
def normalize_datetime_columns(df: pd.DataFrame, columns_names: list[str]) -> pd.DataFrame:
    logging.info(f"\n-> Convertendo colunas de datetime: {columns_names}")
    for name in columns_names:
        df[name] = pd.to_datetime(
            df[name], unit='s', utc=True).dt.tz_convert('America/Sao_Paulo')
